...
print(names(smoke_updated_no_outliers))

smoke_detection$Fire.Alarm <- factor(smoke_detection$Fire.Alarm, levels = c(0, 1), labels = c("No Fire", "Fire"))
#smoke_updated_no_outliers$Fire.Alarm <- factor(smoke_updated_no_outliers$Fire.Alarm, levels = c(0, 1), labels = c("No Fire", "Fire"))
str(smoke_detection)
str(smoke_updated_no_outliers)
summary(smoke_detection)

# Loop through each column name
for (col_name in names(smoke_updated_no_outliers)) {
  # Check if the column is numeric and not in the exclude list
  if (is.numeric(smoke_updated_no_outliers[[col_name]]) && !(col_name %in% exclude_columns)) {
    # Calculate Q1, Q3, and IQR for the current column
    Q1 <- quantile(smoke_updated_no_outliers[[col_name]], 0.25, na.rm = TRUE)
    Q3 <- quantile(smoke_updated_no_outliers[[col_name]], 0.75, na.rm = TRUE)
    IQR_value <- Q3 - Q1
    # Define the lower and upper bounds for outliers
    lower_bound <- Q1 - 1.5 * IQR_value
    upper_bound <- Q3 + 1.5 * IQR_value
    # Filter out the rows that have outliers in the current column
    smoke_updated_no_outliers <- smoke_updated_no_outliers[
      smoke_updated_no_outliers[[col_name]] >= lower_bound & smoke_updated_no_outliers[[col_name]] <= upper_bound, ]
    print(paste("Rows after outlier removal in column", col_name, ":", nrow(smoke_updated_no_outliers)))
    user_input <- readline(prompt = "Please enter something: ")
    
  } else {
    # Print a message if the column is excluded or not numeric
    if (col_name %in% exclude_columns) {
      print(paste("Skipping outlier removal for excluded column:", col_name))
    } else {
...
# Set up the plotting area for 4 boxplots in a 2x2 grid
par(mfrow = c(2, 2))  # 2 rows and 2 columns for 4 plots

# Plot the boxplots
boxplot(smoke_detection$Temperature.C., 
        main = "Temperature.C.", 
        col = "lightblue", 
        border = "black", 
        ylab = "Temperature (°C)")

boxplot(smoke_detection$Humidity..., 
        main = "Humidity...", 
        col = "lightgreen", 
        border = "black", 
        ylab = "Humidity (%)")

boxplot(smoke_detection$TVOC.ppb., 
        main = "TVOC.ppb.", 
        col = "lightcoral", 
        border = "black", 
        ylab = "TVOC (ppb)")

boxplot(smoke_detection$Raw.H2, 
        main = "Raw.H2", 
        col = "lightyellow", 
        border = "black", 
        ylab = "Raw.H2")

# Reset layout to default (one plot at a time)
par(mfrow = c(1, 1))
...
# Plot the boxplots
boxplot(smoke_updated_no_outliers$Temperature.C., 
        main = "Temperature.C.", 
        col = "lightblue", 
        border = "black", 
        ylab = "Temperature (°C)")

boxplot(smoke_updated_no_outliers$Humidity..., 
        main = "Humidity...", 
        col = "lightgreen", 
        border = "black", 
        ylab = "Humidity (%)")

boxplot(smoke_updated_no_outliers$TVOC.ppb., 
        main = "TVOC.ppb.", 
        col = "lightcoral", 
        border = "black", 
        ylab = "TVOC (ppb)")

boxplot(smoke_updated_no_outliers$Raw.H2, 
        main = "Raw.H2", 
        col = "lightyellow", 
        border = "black", 
        ylab = "Raw.H2")
...
train = sample(1:nrow(smoke_updated_no_outliers), nrow(smoke_updated_no_outliers)*(2/3)) # replace=F by default
train
smoke_updated_no_outliers.train = smoke_updated_no_outliers[train,]
smoke_updated_no_outliers.test = smoke_updated_no_outliers[-train,] 
fit = rpart(Fire.Alarm ~., # formula
            data=smoke_updated_no_outliers.train, # dataframe used
...
library(corrplot)
cor_matrix <- cor(smoke_updated_no_outliers, use = "complete.obs")
print(cor_matrix)
...
smoke_detection.pred <- predict(fit,smoke_updated_no_outliers.train, type="class")
smoke_detection.actual <- smoke_updated_no_outliers.train$Fire.Alarm
confusion.matrix1 <- table(smoke_detection.pred, smoke_detection.actual)
pt <- prop.table(confusion.matrix1)
confusion.matrix1
#accuracy
pt[1,1] + pt[2,2]
#error rate
...
tp1 <- confusion.matrix1[2,2]
tn1 <- confusion.matrix1[1,1]
fp1 <- confusion.matrix1[2,1]
fn1 <- confusion.matrix1[1,2]
...
cat("True Negative Rate (TNR):", tnr1, "\n")
cat("False Positive Rate (FPR):", fpr1, "\n")
cat("False Negative Rate (FNR):", fnr1, "\n")


smoke_detection.pred <- predict(fit, smoke_updated_no_outliers.test, type="class")
smoke_detection.actual <- smoke_updated_no_outliers.test$Fire.Alarm
confusion.matrix2 <- table(smoke_detection.pred, smoke_detection.actual)
confusion.matrix2

pt <- prop.table(confusion.matrix2)
#accuracy
pt[1,1] + pt[2,2]


tp2 <- confusion.matrix2[2,2]
tn2 <- confusion.matrix2[1,1]
fp2 <- confusion.matrix2[2,1]
fn2 <- confusion.matrix2[1,2]
...
#train <- sample(1:nrow(smoke_updated_no_outliers), (2/3)*nrow(smoke_updated_no_outliers))
#train.data <- smoke_updated_no_outliers[train,]
#test.data <- smoke_updated_no_outliers[-train,]
...
#smoke_updated_no_outliers$Fire.Alarm <- as.factor(smoke_updated_no_outliers$Fire.Alarm)
#logit.reg <- glm(Fire.Alarm ~.,
#data = smoke_updated_no_outliers.test, family = "binomial")
#summary(logit.reg)
